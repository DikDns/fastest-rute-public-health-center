...
// Konstruktor FPTree
FPTree::FPTree(const std::vector<Transaction> &transactions, uint64_t minimum_support_threshold) : root(std::make_shared<FPNode>(Item{}, nullptr)), header_table(), minimum_support_threshold(minimum_support_threshold)
{
  ...
  // Hapus item yang frekuensinya kurang dari minimum support threshold
  ...
  // Urutkan item berdasarkan frekuensi secara menurun
